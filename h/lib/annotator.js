...
(function() {
  ...
  Util.addToSet = function(element, set) {
    if (__indexOf.call(set, element) < 0) {
      return set.push(element);
    }
  };

  Util.removeFromSet = function(element, set) {
    var index, _ref1;
    index = set.indexOf(element);
    if (index !== -1) {
      [].splice.apply(set, [index, index - index + 1].concat(_ref1 = [])), _ref1;
    }
    return null;
  };
  ...
  Anchor = (function() {
    function Anchor(annotator, annotation, target, startPage, endPage, quote, diffHTML, diffCaseOnly) {
      var pageIndex, _base, _k, _ref1, _ref2;
      ...
      if (this.diffHTML) {
        ...
        delete this.diffHTML;
      }
      if (this.diffCaseOnly) {
        this.target.diffCaseOnly = this.diffCaseOnly;
      }
      delete this.diffCaseOnly;
      this.annotation.anchors.push(this);
      Util.removeFromSet(this.annotation, this.annotator.orphans);
      if (this.annotation.anchors.length === this.annotation.target.length) {
        Util.removeFromSet(this.annotation, this.annotator.halfOrphans);
      } else {
        Util.addToSet(this.annotation, this.annotator.halfOrphans);
      }
      for (pageIndex = _k = _ref1 = this.startPage, _ref2 = this.endPage; _ref1 <= _ref2 ? _k <= _ref2 : _k >= _ref2; pageIndex = _ref1 <= _ref2 ? ++_k : --_k) {
        if ((_base = this.annotator.anchors)[pageIndex] == null) {
          _base[pageIndex] = [];
        }
        this.annotator.anchors[pageIndex].push(this);
      }
    }
    ...
    Anchor.prototype.realize = function() {
      var created, error, hlError, p, page, pagesTodo, promises, renderedPages, _k, _l, _len2, _len3, _m, _ref1, _ref2, _ref3, _results, _results1,
        _this = this;
      ...
      try {
        ...
        for (_l = 0, _len2 = pagesTodo.length; _l < _len2; _l++) {
          page = pagesTodo[_l];
          promises.push(p = this._createHighlight(page));
          p.then(function(hl) {
            return created.push(_this.highlight[page] = hl);
          });
        }
        return (_ref3 = Annotator.$).when.apply(_ref3, promises).always(function() {
          _this.fullyRealized = renderedPages.length === _this.endPage - _this.startPage + 1;
        ...
        _results1 = [];
        for (_m = 0, _len3 = pagesTodo.length; _m < _len3; _m++) {
          page = pagesTodo[_m];
    ...
    Anchor.prototype.remove = function() {
      var anchors, index, _k, _ref1, _ref2;
      ...
      Util.removeFromSet(this, this.annotation.anchors);
      if (this.annotation.anchors.length) {
        return Util.addToSet(this.annotation, this.annotator.halfOrphans);
      } else {
        Util.addToSet(this.annotation, this.annotator.orphans);
        return Util.removeFromSet(this.annotation, this.annotator.halfOrphans);
      }
    };
  ...
  Annotator = (function(_super) {
    ...
    Annotator.prototype.selectedData = null;
    ...
    Annotator.prototype.createAnnotation = function() {
      ...
      if (this.selectedData) {
        Annotator.$.extend(annotation, this.selectedData);
    ...
    Annotator.prototype._createAnchorWithStrategies = function(annotation, target, strategies, promise) {
    ...
    Annotator.prototype._createAnchor = function(annotation, target) {
      var dfd;
      if (target == null) {
    ...
    Annotator.prototype._findAnchorForTarget = function(annotation, target) {
      var anchor, _k, _len2, _ref1;
      _ref1 = annotation.anchors;
      for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
        anchor = _ref1[_k];
    ...
    Annotator.prototype._hasAnchorForTarget = function(annotation, target) {
      var anchor;
      anchor = this._findAnchorForTarget(annotation, target);
      return anchor != null;
    };

    Annotator.prototype._anchorAnnotation = function(annotation, targetFilter, publishEvent) {
      var dfd, index, promises, shouldDo, t, _ref1,
        _this = this;
      ...
      dfd = Annotator.$.Deferred();
      (_ref1 = Annotator.$).when.apply(_ref1, promises).always(function() {
        var p;
        ...
        if (__indexOf.call((function() {
          var _k, _len2, _results;
          _results = [];
          for (_k = 0, _len2 = promises.length; _k < _len2; _k++) {
            p = promises[_k];
    ...
    Annotator.prototype._anchorAllAnnotations = function(targetFilter) {
      var annotation, annotations, dfd, promises, _ref1,
        _this = this;
      dfd = Annotator.$.Deferred();
      ...
      loader = function(annList) {
        ...
        promises = (function() {
          ...
          for (_k = 0, _len2 = now.length; _k < _len2; _k++) {
            n = now[_k];
            _results.push(this.setupAnnotation(n));
          }
          return _results;
        }).call(_this);
    ...
    Annotator.prototype.onSuccessfulSelection = function(event, immediate) {
      ...
      this.selectedData = event.annotationData;
      if (immediate) {
    ...
    Annotator.prototype.onFailedSelection = function(event) {
      this.adder.hide();
      this.selectedTargets = [];
      return delete this.selectedData;
    };
    ...
    Annotator.prototype.onAdderClick = function(event) {
      ...
      return this.setupAnnotation(annotation).then(function(annotation) {
        var anchor, cancel, cleanup, hl, page, save, _k, _len2, _ref1, _ref2;
        _ref1 = annotation.anchors;
        for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
          anchor = _ref1[_k];
        ...
        _this.subscribe('annotationEditorHidden', cancel);
        _this.subscribe('annotationEditorSubmit', save);
        return _this.showEditor(annotation, position);
      });
    };
