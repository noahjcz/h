...
(function() {
  ...
  TextPositionAnchor = (function(_super) {
    ...
    TextPositionAnchor.prototype._createHighlight = function(page) {
      ...
      mappings = this.annotator.anchoring.domMapper.getMappingsForCharRange(this.start, this.end, [page]);
  ...
  Annotator.Plugin.TextAnchors = (function(_super) {
    ...
    TextAnchors.prototype.checkDTM = function() {
      var _ref1;
      return this.useDTM = ((_ref1 = this.annotator.anchoring.domMapper) != null ? _ref1.getCorpus : void 0) != null;
    };
    ...
    TextAnchors.prototype.pluginInit = function() {
      ...
      this.$ = Annotator.$;
      this.annotator.anchoring.anchoringStrategies.push({
        name: "range",
    ...
    TextAnchors.prototype._getTextQuoteSelector = function(range) {
      ...
      if (this.useDTM) {
        startOffset = (this.annotator.anchoring.domMapper.getInfoForNode(rangeStart)).start;
        endOffset = (this.annotator.anchoring.domMapper.getInfoForNode(rangeEnd)).end;
        quote = this.annotator.anchoring.domMapper.getCorpus().slice(startOffset, +(endOffset - 1) + 1 || 9e9).trim();
        _ref1 = this.annotator.anchoring.domMapper.getContextForCharRange(startOffset, endOffset), prefix = _ref1[0], suffix = _ref1[1];
        return {
    ...
    TextAnchors.prototype._getTextPositionSelector = function(range) {
      ...
      startOffset = (this.annotator.anchoring.domMapper.getInfoForNode(range.start)).start;
      endOffset = (this.annotator.anchoring.domMapper.getInfoForNode(range.end)).end;
      return {
    ...
    TextAnchors.prototype.verifyTextAnchor = function(anchor, reason, data) {
      ...
      corpus = this.annotator.anchoring.domMapper.getCorpus();
    ...
    TextAnchors.prototype.createFromRangeSelector = function(annotation, target) {
      ...
      if (this.useDTM) {
        startInfo = this.annotator.anchoring.domMapper.getInfoForNode(normedRange.start);
        startOffset = startInfo.start;
        if (startOffset == null) {
          dfd.reject("the saved quote doesn't match");
          return dfd.promise();
        }
        endInfo = this.annotator.anchoring.domMapper.getInfoForNode(normedRange.end);
        endOffset = endInfo.end;
        if (endOffset == null) {
          dfd.reject("the saved quote doesn't match");
          return dfd.promise();
        }
        q = this.annotator.anchoring.domMapper.getCorpus().slice(startOffset, endOffset).trim();
    ...
    TextAnchors.prototype.createFromPositionSelector = function(annotation, target) {
      ...
      content = this.annotator.anchoring.domMapper.getCorpus().slice(selector.start, +(selector.end - 1) + 1 || 9e9).trim();
      currentQuote = this.annotator.normalizeString(content);
      savedQuote = this.getQuoteForTarget(target);
      if ((savedQuote != null) && currentQuote !== savedQuote) {
        dfd.reject("the saved quote doesn't match");
      ...
      dfd.resolve(new TextPositionAnchor(this.annotator, annotation, target, selector.start, selector.end, this.annotator.anchoring.domMapper.getPageIndexForPos(selector.start), this.annotator.anchoring.domMapper.getPageIndexForPos(selector.end), currentQuote));
      return dfd.promise();
    };
